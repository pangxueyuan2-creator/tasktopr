...
Expected terminal phases are generated by the live run: `ANALYZING_ISSUE`, `SCANNING_REPOSITORY`, `CREATING_PLAN`, `CREATING_BRANCH`, `EDITING_FILE`, `RUNNING_TEST`, `REVIEWING_PATCH`, and `COMPLETED`. The script then displays `git diff` and re-runs `pytest`; it does not print a fabricated transcript.
